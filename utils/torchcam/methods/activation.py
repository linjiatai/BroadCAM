...
class CAM(_CAM):
    ...
    def __init__(
        ...
        fc_layer: Optional[Union[nn.Module, str]] = 'fc8',
...
class ScoreCAM(_CAM):
    ...
    def _get_score_weights(self, activations: List[Tensor], class_idx: Union[int, List[int]]) -> List[Tensor]:
        ...
        self._input = self._input.cpu()
        scored_inputs = [
            (act.unsqueeze(2).cpu() * self._input.unsqueeze(1)).view(b * c, *self._input.shape[1:]) for act in activations
        ]
        gpu=activations[0].device.index
        # Initialize weights
        # (N * C)
        weights = [torch.zeros(b * c, dtype=t.dtype).to(device=t.device) for t in activations]
        # weights = [torch.zeros(b * c, dtype=t.dtype) for t in activations]

        # (N, M)
        logits = self.model(self._input.to(gpu))
        ...
        for idx, scored_input in enumerate(scored_inputs):
            ...
            for _idx in range(math.ceil(weights[idx].numel() / self.bs)):
                ...
                # (*, M)
                cic = self.model(scored_input[_slice].to(gpu)) - logits[idcs[_slice]]
                if isinstance(class_idx, int):
